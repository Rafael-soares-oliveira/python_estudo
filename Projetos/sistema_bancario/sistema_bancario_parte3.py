    ...
    def data_nascimento(self):
        while True:
            try:
                ...
                self.ANIVERSARIO = str(x4.day).zfill(2) + '-' + str(x4.month).zfill(2) + '-' + str(x4.year)
...
class Conta_pessoa_fisica():
    ...
    def password(self):
        while True:
            ...
            confirm_password = maskpass.askpass(prompt='CONFIRMAÇÂO PASSWORD: ',mask="*")
    ...
    def data_fundacao(self):
        while True:
            try:
                ...
                x4 = date(x3,x2,x1) # Transform inputs in date format
                self.FUNDACAO = str(x4.day).zfill(2) + '-' + str(x4.month).zfill(2) + '-' + str(x4.year)
...
def cadastro_contas_pessoa_fisica(variavel,tipo_conta,cadastro,contas,caminho_cadastro,caminho_contas,extrato,caminho_extrato):
...
def cadastro_contas_pessoa_juridica(variavel,tipo_conta,cadastro,contas,caminho_cadastro,caminho_contas,extrato,caminho_extrato):
    variavel2 = vars(tipo_conta)
...
def sacar(acesso,lista_pf,y,path_conta_pf,lista_pj,path_conta_pj,list_extrato,path_extrato,horario):
    os.system('clear')
    print('\n======== SAQUE ========')
    print()
...
def depositar(acesso,lista_pf,y,path_conta_pf,lista_pj,path_conta_pj,list_extrato,path_extrato,horario):
    os.system('clear')
    print('\n======== DEPÓSITO ========')
    print()
...
def main():
    ...
    PATH_FILE_CADASTROS_PF = './Projetos/sistema_bancario/cadastros_pf.json' # Caminho do arquivo JSON que possui todos os cadastros de pessoa física
    PATH_FILE_CADASTROS_PJ = './Projetos/sistema_bancario/cadastros_pj.json' # Caminho do arquivo JSON que possui todos os cadastros de pessoa jurídica
    PATH_FILE_CONTAS_PF = './Projetos/sistema_bancario/contas_pf.json' # Caminho do arquivo JSON que possui todos as contas de pessoa física
    PATH_FILE_CONTAS_PJ = './Projetos/sistema_bancario/contas_pj.json' # Caminho do arquivo JSON que possui todos as contas de pessoa jurídica
    PATH_FILE_NUMEROS = './Projetos/sistema_bancario/numeros.json' # Caminho do arquivo JSON que armazena o último número de conta criado. Não haverá duplicados.
    PATH_FILE_EXTRATOS = './Projetos/sistema_bancario/extratos.json' # Caminho do arquivo JSON que armazena todas as operações feitas pelo usuário da conta.
    lista_cadastros_pf = read([],PATH_FILE_CADASTROS_PF)
    lista_cadastros_pj = read([],PATH_FILE_CADASTROS_PJ)
    lista_contas_pf = read([],PATH_FILE_CONTAS_PF)
    lista_contas_pj = read([],PATH_FILE_CONTAS_PJ)
    listas_numeros = read((1),PATH_FILE_NUMEROS)
    listas_extratos = read([],PATH_FILE_EXTRATOS)
    format = "%d-%m-%Y %H:%M:%S" # Define o formato da data
    horario_atual = datetime.now(timezone('UTC'))
    agora_brasil = horario_atual.astimezone(timezone('America/Sao_Paulo')) # Horário atual no fuso-horário do Brasil/São Paulo
    ...
    while True:
        ...
        if option1 == '1': # Cadastrar
            ...
            if option2 == '1': # Pessoa Física
                ...
                while True:
                    ...
                    if option3 == '1': # Conta Salário
                        cadastro_contas_pessoa_fisica(p,Conta_pessoa_fisica.conta_salario(str(listas_numeros).zfill(4)),
                        lista_cadastros_pf,lista_contas_pf,PATH_FILE_CADASTROS_PF,PATH_FILE_CONTAS_PF,listas_extratos,PATH_FILE_EXTRATOS)
                        break
                        
                    elif option3 == '2': # Conta Corrente Padrão
                        cadastro_contas_pessoa_fisica(p,Conta_pessoa_fisica.conta_corrente_padrao(str(listas_numeros).zfill(4)),
                            lista_cadastros_pf,lista_contas_pf,PATH_FILE_CADASTROS_PF,PATH_FILE_CONTAS_PF,listas_extratos,PATH_FILE_EXTRATOS)
                        break
                        
                    elif option3 == '3': # Conta Corrente Premium
                        cadastro_contas_pessoa_fisica(p,Conta_pessoa_fisica.conta_corrente_premium(str(listas_numeros).zfill(4)),
                            lista_cadastros_pf,lista_contas_pf,PATH_FILE_CADASTROS_PF,PATH_FILE_CONTAS_PF,listas_extratos,PATH_FILE_EXTRATOS)
                        break
                        
                    elif option3 == '4': # Conta Poupança
                        cadastro_contas_pessoa_fisica(p,Conta_pessoa_fisica.conta_poupanca(str(listas_numeros).zfill(4)),
                            lista_cadastros_pf,lista_contas_pf,PATH_FILE_CADASTROS_PF,PATH_FILE_CONTAS_PF,listas_extratos,PATH_FILE_EXTRATOS)
                        break
                ...
                listas_numeros = int(listas_numeros) + 1
                save(listas_numeros,PATH_FILE_NUMEROS)
            ...
            if option2 == '2': # Pessoa Jurídica
                ...
                while True:
                    ...
                    if option3 == '1': # Conta Jurídica Padrão
                        cadastro_contas_pessoa_juridica(p,Conta_pessoa_juridica.conta_juridica_padrao(str(listas_numeros).zfill(4)),
                            lista_cadastros_pj,lista_contas_pj,PATH_FILE_CADASTROS_PJ,PATH_FILE_CONTAS_PJ,listas_extratos,PATH_FILE_EXTRATOS)
                        break
                    
                    elif option3 == '2': # Conta Jurídica Premium
                        cadastro_contas_pessoa_juridica(p,Conta_pessoa_juridica.conta_juridica_premium(str(listas_numeros).zfill(4)),
                            lista_cadastros_pj,lista_contas_pj,PATH_FILE_CADASTROS_PJ,PATH_FILE_CONTAS_PJ,listas_extratos,PATH_FILE_EXTRATOS)
                        break
        ...
        elif option1 == '2' and listas_extratos == []: # Tentar acessar alguma conta porém não há alguma conta cadastrada
            os.system('clear')
            ...
            tentativa = 1
            while True:
                x,y = acessar_conta(tentativa,Menu.login(),lista_contas_pf,lista_contas_pj) # x é a conta acessada e y será utilizado para diferenciar entre PF e PJ
                for z in x: # Retira os dados de dentro do parentêses e transforma em dicionário. Mais simples de manipular
                    conta_acessada = z

                tentativa +=1
                if x != 0: # Toda tentativa incorreta irá retornar x == 0
                    break
                
            while True:
                option2 = Menu.operacoes()
                if option2 == '1': # Operação de Sacar
                    sacar(conta_acessada,lista_contas_pf,y,PATH_FILE_CONTAS_PF,lista_contas_pj,PATH_FILE_CONTAS_PJ,
                        listas_extratos,PATH_FILE_EXTRATOS,str(agora_brasil.strftime(format)))
                
                elif option2 == '2': # Operação de Depositar
                    depositar(conta_acessada,lista_contas_pf,y,PATH_FILE_CONTAS_PF,lista_contas_pj,PATH_FILE_CONTAS_PJ,
                        listas_extratos,PATH_FILE_EXTRATOS,str(agora_brasil.strftime(format)))
                    
                elif option2 == '3': # Operação de Consultar Extrato
                    consulta_extrato(listas_extratos,conta_acessada)
                    
                elif option2 == '4': # Operação de Consultar Saldo
                    saldo(conta_acessada)
                    
                elif option2 == '5': # Operação de Consultar Dados Cadastrais
                    consulta_cadastro(y,lista_cadastros_pf,lista_cadastros_pj,conta_acessada)
